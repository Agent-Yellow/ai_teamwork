...
ROOT="${AI_OPERATOR_ROOT:-$HOME/ai_teamwork/ai-operator}"

echo "Initiating AI Operator Emergency Stop..."

# 1. Remove the crontab entries to prevent heartbeat/nightly restarts
crontab -l | grep -v "$ROOT" | grep -v 'ai-operator' | crontab -
echo "Cron entries removed."

# 2. Kill any currently running execution scripts
pkill -f "$ROOT/control_plane/scripts" || true
echo "Running operator processes killed."

# 3. Freeze active jobs (backup first, then clear active queues)
mkdir -p "$ROOT/jobs"
cp "$ROOT/jobs/active.json" "$ROOT/jobs/active_frozen.json.bak" 2>/dev/null || true
echo '{"queued": [], "running": [], "failed": [], "retryable": [], "awaiting_approval": [], "assigned": []}' > "$ROOT/jobs/active.json"
